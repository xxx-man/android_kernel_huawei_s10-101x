...
static int g_cfg80211_up = TRUE;
#define MAC2STR(a) (a)[0], (a)[1], (a)[2], (a)[3], (a)[4], (a)[5]
#define MACSTR "%02x:%02x:%02x:%02x:%02x:%02x"
#define MAX_WAIT_TIME 3000 /* wait more time than 1500 to avoid IF-DEL conflict */
#define WL_SCAN_ACTIVE_TIME	 40
#define WL_SCAN_PASSIVE_TIME	130
#define WL_FRAME_LEN			512
...
fail:
    ...
    WL_ERR(("%s: virtual interface(%s) is not created \n", __func__, wl->p2p->vir_ifname));
    memset(wl->p2p->vir_ifname, '\0', IFNAMSIZ);
...
#ifdef WL_PROTECT
			/* reload firmware if delete error happens */
			if (ret) {
				WL_ERR(("Firmware returned an error (%d) from p2p_ifdel, reload fw now...\n", ret));
...
static s32 wl_inform_single_bss(struct wl_priv *wl, struct wl_bss_info *bi)
{
	...
	if (wl->p2p && wl->p2p_net && wl->scan_request && wl->p2p->bss_idx &&
	        (wl->p2p->bss_idx[P2PAPI_BSSCFG_CONNECTION].dev) &&
	...
	const wl_event_msg_t *e, void *data)
{
	...
	switch (event) {
		...
		case WLC_E_REASSOC_IND:
				printk("change the reassoc frame to assoc frame: BCM for APPLE PC \n");
				fc = FC_ASSOC_REQ;
			break;
	...
	isfree = true;
		...
		if ((event == WLC_E_REASSOC_IND || event == WLC_E_ASSOC_IND) && reason == DOT11_SC_SUCCESS) {
	...
	const wl_event_msg_t *e, void *data)
{
	...
	if(unlikely(!g_cfg80211_up))
	{
		printk(KERN_ERR "%s:driver is not up yet, g_cfg80211_up=%d\n", __FUNCTION__, g_cfg80211_up);
		return -EINVAL;
	}
	...
	while (down_interruptible (&tsk->sema) == 0) {
		...
		while ((e = wl_deq_event(wl))) {
			...
			if (e->etype < WLC_E_LAST && wl->evt_handler[e->etype]) {
				if ( unlikely( (netdev == NULL) || (!g_cfg80211_up)) )
					WL_ERR(("Event(%d) ignored, netdev is %s, g_cfg80211_up=%d\n", e->etype, (netdev == NULL)?"NULL":"not NULL", g_cfg80211_up));
				else
